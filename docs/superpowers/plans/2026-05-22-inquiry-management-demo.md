...
### 設計上の判断（実装者向けメモ）
- **カテゴリ／ステータスの `<select>` は native select を使う**（shadcn の Select は Radix の制御コンポーネントで、`<Form method="post">` の素のフォーム送信に値を載せるには hidden input 等の追加配線が必要。デモでは native select を Tailwind で整形する方が堅牢で短い）。Button / Input / Textarea / Table / Badge / Card は shadcn を使う。
...
- [ ] **Step 1: 一時ディレクトリに C3 で雛形生成**

```bash
cd /Users/fshindo/work
bun create cloudflare@latest mihia-inquiry-tmp --framework=react-router --git=false --deploy=false
```

対話プロンプトが出た場合の回答: TypeScript = yes / git = no / deploy(Do you want to deploy) = no。生成完了後 `mihia-inquiry-tmp/` に React Router v7 + Cloudflare Vite プラグイン + Tailwind 同梱の雛形ができる。

- [ ] **Step 2: 雛形をリポジトリへ移動（docs/ と .git は維持）**
...
Expected: エラーなしで完了（テンプレートに `typecheck` スクリプトが無い場合は `bunx tsc --noEmit` を実行）。
...
Run: `bun run test`
Expected: 「No test files found」で正常終了（テスト未作成のため）。
...
export const STATUS_BADGE_CLASS: Record<Status, string> = {
  未対応: "bg-gray-200 text-gray-800 hover:bg-gray-200",
  対応中: "bg-blue-200 text-blue-800 hover:bg-blue-200",
  完了: "bg-green-200 text-green-800 hover:bg-green-200",
};
```
...
`Env` は Step 7 の `wrangler types` で生成される global 型。
...
Expected: `drizzle/migrations/` に `0000_*.sql`（`inquiries` テーブルの CREATE 文と CHECK 制約を含む）が生成される。中身を開き、`CHECK (category IN ('総務', 'IT', '人事'))` 等が含まれることを確認。
...
この時点では `thanks` / `admin` / `admin-detail` が未作成なのでビルドは通らない。続く Task で作成する（このコミットは Task 7 末尾、home 作成後）。
      ...
      <Card>
        ...
        <CardContent>
          <Form method="post" className="space-y-4">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium">登録者名</label>
  ...
  return (
    <main className="mx-auto max-w-4xl p-6">
      ...
      <Form method="get" className="mb-4 flex items-center gap-2">
        <label className="text-sm">ステータス絞り込み:</label>
        <select
          name="status"
          ...
          {rows.length === 0 ? (
            ...
            rows.map((row) => (
              <TableRow key={row.id}>
                ...
                <TableCell>
                  <Badge className={STATUS_BADGE_CLASS[row.status as keyof typeof STATUS_BADGE_CLASS]}>
                    {row.status}
                  </Badge>
                </TableCell>
...
- [ ] **Step 1: 詳細画面・loader・更新 action を作成**
...
```bash
git add app/routes/admin-detail.tsx
...
- [ ] **Step 1: 生成済み ErrorBoundary を確認**

`app/root.tsx` を開く。C3 テンプレートは `isRouteErrorResponse(error)` で 404 とその他を出し分ける `ErrorBoundary` を既に含む。404 のとき「404 / The requested page could not be found.」等を表示する作りになっている。

- [ ] **Step 2: 404 のメッセージを日本語化（任意の最小調整）**

`ErrorBoundary` 内の 404 分岐の文言を日本語にする。例（既存の該当箇所を置き換え）:
...
（テンプレートの変数名 `message` / `details` に合わせる。異なる場合は既存の構造に沿って文言だけ差し替える。）

- [ ] **Step 3: 404 表示を確認**

Run: `bun run dev`
手順: `/admin/9999` で日本語の 404 メッセージが表示される。確認後 Ctrl-C。
...
Run: `bun run deploy`（テンプレートに無い場合は `wrangler deploy`）
